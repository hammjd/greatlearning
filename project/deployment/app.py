...
# Load the trained pipeline saved next to this file.
model_path = os.path.join(os.path.dirname(__file__), "model.joblib")
model = joblib.load(model_path)

st.title("Tourism Package Prediction")
st.write(
    "Fill in the customer details below to predict "
    "whether they'll purchase a travel package."
)
...
Designation = st.selectbox(
    "Designation", ["Executive", "Manager", "AVP", "VP", "Sr. Manager"]
)
MonthlyIncome = st.number_input(
    "Monthly Income", min_value=1000.0, value=30000.0
)

# Use the same feature names and raw categories as training.
...
if st.button("Predict"):
    ...
    result = (
        "is likely to purchase the travel package"
        if pred == 1
        else "is unlikely to purchase the travel package"
    )
